...
type PairOpts = {user: string; host: string; onPaired: (userHost: string) => void};
...
export function openPairSheet(opts: PairOpts): HTMLDivElement {
  const userHost = `${opts.user}@${opts.host}`;
  const theme = currentPresentationTheme();
  const element = h(
    'div',
    'cyc-modal cyc-modal-dialog cyc-pair-sheet fixed inset-0 z-[15] m-0 flex overflow-auto bg-[rgba(0,0,0,0.3)] p-[1.875rem] text-[1rem] ' +
      DIALOG_TEXT[theme]
  );
  const container = h(
    'div',
  ...
  desc.textContent =
    `${userHost} needs pairing, which this build does not yet ` +
    'offer. On your own machine the engine and app pair automatically.';
